...
class Recognizer(object):
    ...
    def loadFaceDB(self, directory = 'facedb'):
        ...
        if False == exists(directory) || False == isdir(dirrectory):
            ...
            if isdir(join(directory,f)):
                ...
                for img in listdir(join(directory,f)):
                    if False == isdir(join(directory,f,img)):
                        ...
                        length = np.max(wh);
                        center = np.round((upperleft + downright) / 2);
                        upperleft = center - np.round(np.array([length,length]) / 2);
                        downright = upperleft + np.array([length,length]);
                        face = image[upperleft[1]:downright[1],upperleft[0]:downright[0],:];
                        imgs.append(face);
                feature = self.encoder.encode(imgs);
                label = tf.tile(tf.constant([[label]], dtype = tf.int32), (features.shape[0],1));
    ...
    def recognize(self, image):
        ...
        upperleft = center - tf.stack([length,length], axis = -1);
